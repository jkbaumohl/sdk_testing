package JB_Contig_Test::JB_Contig_TestClient;
...
JB_Contig_Test::JB_Contig_TestClient
...
sub new
{
    ...
    my $self = {
	client => JB_Contig_Test::JB_Contig_TestClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$params is a JB_Contig_Test.FilterContigsParams
$return is a JB_Contig_Test.FilterContigsResults
FilterContigsParams is a reference to a hash where the following keys are defined:
	workspace has a value which is a JB_Contig_Test.workspace_name
	contigset_id has a value which is a JB_Contig_Test.contigset_id
...
FilterContigsResults is a reference to a hash where the following keys are defined:
	...
	new_contigset_ref has a value which is a JB_Contig_Test.ws_contigset_id
...
# Authentication: required
    ...
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "JB_Contig_Test.filter_contigs",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for JB_Contig_Test::JB_Contig_TestClient\n";
    }
    if ($sMajor == 0) {
        warn "JB_Contig_Test::JB_Contig_TestClient version is $svr_version. API subject to change.\n";
    }
}
...
package JB_Contig_Test::JB_Contig_TestClient::RpcClient;
