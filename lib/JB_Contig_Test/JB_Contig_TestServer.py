...
def get_config():
    ...
    for nameval in config.items(get_service_name() or 'JB_Contig_Test'):
        retconfig[nameval[0]] = nameval[1]
    return retconfig

config = get_config()

from JB_Contig_Test.JB_Contig_TestImpl import JB_Contig_Test
impl_JB_Contig_Test = JB_Contig_Test(config)
...
class Application(object):
    ...
    def __init__(self):
        submod = get_service_name() or 'JB_Contig_Test'
        ...
        self.rpc_service.add(impl_JB_Contig_Test.filter_contigs,
                             name='JB_Contig_Test.filter_contigs',
                             types=[dict])
        self.method_authentication['JB_Contig_Test.filter_contigs'] = 'required'
        self.rpc_service.add(impl_JB_Contig_Test.status,
                             name='JB_Contig_Test.status',
                             types=[dict])
                    ...
                    if auth_req != "none":
                        if token is None and auth_req == 'required':
                            ...
                            err.data = "Authentication required for " + \
                                "JB_Contig_Test but no authentication header was passed"
                            raise err
